...
- You can pass additional properties (e.g. `prefix`) to the config object. The extended config will be passed to plugins, allowing further customization of plugin output. Go to the [Custom Plugin](#custom-plugin) section for more details.
...
With `outputVariable: false`, the plugin resolves any reference into CSS value if the referenced design token has no variation.
...
With `outputVariable: true`, the plugin converts any reference into a CSS custom property.
...
Note that our plugin does not scan HTML/JS files and thus, cannot pick up CSS custom properties from inline styles. You need to pass those to the plugin's `safelist` option to ensure the relevant CSS rules are generated.
...
```json
{
  "plugins": {
    ...
    "theminglayer/postcss": {
      "outputVariable": false,
      "safelist": ["var(--tl-border-color-primary)"]
    }
  }
}
```
...
// plugins and outDir are not required
export default defineConfig({
  sources: `design-tokens.json`,
  prefix: `tl-`,
})
...
- `buildOptions` config object, excluding `sources` and `plugins`
